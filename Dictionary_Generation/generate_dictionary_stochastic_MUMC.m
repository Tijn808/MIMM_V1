function dictionary = generate_dictionary_stochastic_MUMC()
% Dictionary Generation for MIMM — MUMC acquisition protocol
% TEs match MUMC ME-GRE exactly: [6, 12, 18, 24, 30] ms
% No interpolation needed when used with MIMM on MUMC data.
%
% Based on generate_dictionary_stochastic.m (Mert Sisman 7/27/2023)
...
for q = 1:2000 % Randomly choose simulation input parameters 2000 times for each 2DFD.
...
% Define echo times — MUMC ME-GRE protocol (no interpolation needed).
TE = [6 12 18 24 30] * 1e-3;
% Gyromagnetic ratio.
...
% Fit for R2* using polyfit (no Curve Fitting Toolbox needed).
p = polyfit(TE.', -log(abs(signal.')), 1);
R2s = p(1);
