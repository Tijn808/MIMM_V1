...
Horizontal bar chart of mean MVF ± SD per ROI, basic (red) vs atlas (blue).
...
Scatter of MVF basic (x) vs MVF atlas (y) per ROI, with identity line and ±1 SD error bars.
...
- **Error bars are very wide** (±SD reaching 0.2–0.5), reflecting that tractography ROIs are large, spatially distributed, and include voxels from multiple adjacent regions, creating high within-ROI variance.

**Interpretation:**
The tractography atlas gives the same overall ranking as DTI-81 but with much wider error bars, indicating it is less precise for individual ROI estimates. The forceps major overestimation at +37% (relative to atlas) mirrors the DTI-81 splenium finding, confirming this is a robust result not dependent on atlas choice.
...
Scatter of MIMM `|chi_myelin|` (ppm, x) vs chi-sep `|χ⁻|` (ppm, y) per ROI, with identity line. Unlike figure 26 (MVF vs |χ⁻|), both axes are the same physical quantity. The identity line shows absolute numerical agreement — points below it mean chi-sep reports larger diamagnetic susceptibility than MIMM.
